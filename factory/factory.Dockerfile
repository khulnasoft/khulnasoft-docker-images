...
# TODO: there are other folders that need permissions but I don't know what they are yet, See: https://github.com/khulnasoft/khulnasoft/issues/23962
RUN ls -la /root \
  && chmod 777 /root \
  && apt-get update \
  && apt-get install --no-install-recommends -y \
    # Always install: Needed to run Khulnasoft
    xvfb \
    libglib2.0-0 \
    libnss3 \
    libatk1.0-0 \
    ...
    libgbm1 \
    libasound2 \
    # Needed to support the ps command, while not used by khulnasoft directly it is used by some of our examples and the dependency is small (~1mb).
    procps \
    # Needed to support curl, similar to ps, it's not directly used by khulnasoft but leaving it in the container is practically free.
    curl \
...
ENV KHULNASOFT_FACTORY_DEFAULT_NODE_VERSION=${FACTORY_DEFAULT_NODE_VERSION}
...
ONBUILD ENV APPLIED_FACTORY_DEFAULT_NODE_VERSION=${NODE_VERSION:-${KHULNASOFT_FACTORY_DEFAULT_NODE_VERSION}}
...
# TODO: Globally installed webkit currently isn't found, see issue https://github.com/khulnasoft/khulnasoft/issues/25344
...
# Install Khulnasoft: optional
ONBUILD ARG KHULNASOFT_VERSION

# Allow projects to reference globally installed khulnasoft
# This is only set if the khulnasoft version is passed in (thats what the + syntax stuff does.)
ONBUILD ENV NODE_PATH=${KHULNASOFT_VERSION:+/usr/local/lib/node_modules}

ONBUILD RUN node /opt/installScripts/khulnasoft/install-khulnasoft-version.js ${KHULNASOFT_VERSION}
...
ONBUILD RUN apt-get purge -y --auto-remove \
  ...
  # Remove khulnasoft install scripts
